...
import { Search, Plus, Filter, MoreVertical, PackageX } from 'lucide-react';

export default function PurchaseReturn() {
  const [searchTerm, setSearchTerm] = useState('');

  const dummyPRtn = [
    { id: 1, date: '28 Sep 2026', prtnNo: 'PRTN-202609-001', vendor: 'PT. Plastikindo', refDoc: 'RI-202609-001', status: 'Selesai' },
    { id: 2, date: '21 Sep 2026', prtnNo: 'PRTN-202609-002', vendor: 'CV. Komputer Jaya', refDoc: 'RI-202609-002', status: 'Menunggu' },
  ];
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Cari no. retur atau vendor..." 
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-slate-950/50 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">No. Retur</th>
                <th className="px-6 py-4 font-medium">Pemasok (Vendor)</th>
                <th className="px-6 py-4 font-medium">Ref. Dokumen</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {dummyPRtn.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                  ...
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{item.refDoc}</td>
