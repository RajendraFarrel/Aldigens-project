...
import { Search, Plus, Filter, MoreVertical, FileCog } from 'lucide-react';

export default function WorkOrder() {
  const [searchTerm, setSearchTerm] = useState('');

  const dummyWO = [
    { id: 1, date: '25 Sep 2026', woNo: 'WO-202609-001', refBOM: 'BOM-202609-001', product: 'Meja Belajar Kayu', targetQty: '50 Unit', status: 'Dalam Proses' },
    { id: 2, date: '24 Sep 2026', woNo: 'WO-202609-002', refBOM: 'BOM-202609-002', product: 'Kursi Kantor Ergonomis', targetQty: '100 Unit', status: 'Selesai' },
    { id: 3, date: '26 Sep 2026', woNo: 'WO-202609-003', refBOM: 'BOM-202609-001', product: 'Meja Belajar Kayu', targetQty: '25 Unit', status: 'Menunggu' },
  ];
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-slate-950/50 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">No. WO</th>
                <th className="px-6 py-4 font-medium">Ref. BOM</th>
                <th className="px-6 py-4 font-medium">Produk Target</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {dummyWO.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                  ...
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{item.refBOM}</td>
                  <td className="px-6 py-4 font-medium">{item.product}</td>
                  <td className="px-6 py-4">{item.targetQty}</td>
